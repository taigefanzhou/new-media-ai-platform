...
| 链接素材采集 | 待凭据 | 否 | 系统支持粘贴链接保存为参考素材；视频号已切换为服务器内置开源解析，配置元宝 Web Cookie 后即可自动下载。 |
...
- 已新增微信开放平台“网站应用”扫码登录链路：
  - 登录页保留账号密码登录，并增加“微信扫码登录”按钮。
  - 管理员在系统设置的“微信登录”中维护 AppID、AppSecret、回调地址、默认角色和启用状态。
  - 公开接口 `/api/auth/wechat/config` 只返回是否启用和 AppID，不返回 AppSecret。
  - `/api/auth/wechat/start` 生成带签名 state 的微信扫码地址。
  - `/api/auth/wechat/callback` 校验 state 后换取微信用户身份；未绑定微信会生成待审批申请。
  - 管理员可批准申请并选择“新建用户”或“绑定已有用户”，批准后同一微信以后可以直接扫码登录。
  - 管理员可拒绝申请，也可停用已绑定微信身份；停用绑定不会删除系统用户。
- 已新增 smoke：`backend/tests/wechat_login_smoke.py`。
  - 覆盖未配置禁用、管理员保存配置、模拟微信回调生成申请、批准申请、再次扫码登录、普通用户素材隔离。
- 数据隔离现状：
  - 普通用户只能看到自己创建的核心业务数据；管理员可以看全局。
  - 现有 helper：`_assign_owner`、`_owned_statement`、`_ensure_record_access`。
  - smoke 已验证一个普通用户创建的素材不会出现在另一个普通用户的素材列表中，且不能被另一个普通用户删除。
- 微信开放平台线上配置要求：
  - 网站应用必须开通微信登录能力。
  - 回调域名填写 `media.tech-ark.com`。
  - 推荐回调地址：`https://media.tech-ark.com/api/auth/wechat/callback`。
...
- 已改进：
  ...
  - 收费解析 API 已停用，后端改为 `provider=yuanbao` 的服务器内置开源解析；凭据只保存元宝 Web Cookie，不在页面展示。
- 下一步只需更新服务器中的元宝 Web Cookie，即可恢复视频号链接自动解析。
...
- “视频任务列表”已把横向滚动限制在表格内部，操作列固定在右侧；分页条固定显示在表格下方，只有一页时上一页/下一页会置灰。
