        ...
        return json.dumps(
            {
                ...
                "requirements": [
                    *reference_analysis_requirements(),
                    ...
                    "每个镜头先给出对应口播、画面文字、人物动作和声音摘要；详细证据与置信度由下一轮复核补充。",
                    "区分内容吸引力和客观播放数据：只能分析留存作用与风险，不得伪造真实完播率或用户数据。",
                    "控制首轮返回体：总长度不超过 7000 个中文字符；timeline、edit_plan、transcript_segments 各不超过 10 段。",
                    "除 transcript 外，长文本字段不超过 300 字；数组内单个文本字段不超过 50 字；空内容用空字符串，不要扩写。",
                    "首轮只输出核心拆解，关键时刻的详细证据、留存曲线和复刻规则由下一轮复核补充。",
